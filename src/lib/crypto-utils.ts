/**
 * KIYOTA ARCHITECTURE: EDGE-SAFE CRYPTO UTILS
 * Purpose: Provides cryptographic functions using Web Crypto API.
 */

// Global crypto is available on Edge and Node 20+. 
// We use a helper for hex generation since native crypto returns Uint8Arrays.
...
/**
 * Pure JS MD5 Implementation for PayHere.
 * Necessary because MD5 is not supported by Web Crypto (crypto.subtle).
 */
export function md5(str: string): string {
  ...
  for (let i = 0; i < byteStr.length; i++) words[i >> 2] |= byteStr[i] << ((i % 4) << 3);
  ...
  return [a, b, c, d]
    .map((v) => (v < 0 ? v + 0x100000000 : v).toString(16).padStart(8, "0").split("").reverse().join("").match(/../g)!.reverse().join(""))
    .join("");
}